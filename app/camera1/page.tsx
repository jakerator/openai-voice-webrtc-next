  ...
  const predictItem = async (photo) => {
    try {
      const response = await fetch("/api/predict", {
        ...
        body: JSON.stringify({ image: photo })  // Send base64 image
      });
      setProcessing(false);
      if (response.ok) {
        ...
        if (data.message.object_on_image) {
          alert_message += "Nice! Object detected on name. \n\n" + data.message.image_desciption;
          alert(alert_message);  // Show predicted item name

          // entering some code;
        }
        else {
          alert_message += "Sorry, but object wasn't detected on your drawing. \n\n" + data.message.image_desciption;
          alert(alert_message);  // Show predicted item name
        }
